...
describe("recipe-utils", () => {
    ...
    describe("convertUnitIfNeeded", function () {
        it("converts tablespoons to teaspoons when below threshold", function () {
            const result = convertUnitIfNeeded(0.5, "tbsp");
            // 0.5 tbsp = 1.5 tsp
            expect(result.amount).toBe(1.5);
            expect(result.unit).toBe("tsp");
        });

        it("converts cups to tablespoons when below threshold", function () {
            const result = convertUnitIfNeeded(0.2, "cup");
            // 0.2 cup = 3.2 tbsp
            expect(result.amount).toBe(3.2);
            expect(result.unit).toBe("tbsp");
        });

        it("converts cups (plural) to tablespoons when below threshold", function () {
            const result = convertUnitIfNeeded(0.2, "cups");
            expect(result.amount).toBe(3.2);
            expect(result.unit).toBe("tbsp");
        });

        it("converts kilograms to grams when below threshold", function () {
            const result = convertUnitIfNeeded(0.05, "kg");
            // 0.05 kg = 50 g
            expect(result.amount).toBe(50);
            expect(result.unit).toBe("g");
        });

        it("doesn't convert when above threshold", function () {
            const tbspResult = convertUnitIfNeeded(1, "tbsp");
            expect(tbspResult.amount).toBe(1);
            expect(tbspResult.unit).toBe("tbsp");

            const cupResult = convertUnitIfNeeded(0.3, "cup");
            expect(cupResult.amount).toBe(0.3);
            expect(cupResult.unit).toBe("cup");
        });

        it("doesn't convert when unit not in conversion list", function () {
            const result = convertUnitIfNeeded(0.1, "gallon");
