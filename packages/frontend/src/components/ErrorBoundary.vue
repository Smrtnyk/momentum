<template>
    <div v-if="error" class="error-boundary">
        <v-card class="error-card ma-4" variant="elevated" elevation="6">
            ...
            <v-card-actions class="error-actions">
                <v-btn color="primary" @click="refreshPage" prepend-icon="mdi-refresh">
                    Refresh Application
                </v-btn>
...
function copyToClipboard(text: string): void {
    navigator.clipboard
        .writeText(text)
        .catch((e) => logger.error("Failed to copy to clipboard:", e));
}
